...
## Why does the web BFF proxy to two services, and how does it reuse one token?

The platform API and the ragent service are separate Fly apps (separate scaling, separate failure domains), but ragent validates the **same HS256 access token** apps/api issues (shared secret). So the BFF doesn't need a second auth flow — `apiFetch`/`apiStream` just take a `base` URL, and two thin proxy routes (`/api/pf-ragent/*` for JSON corpora, `/api/pf-ragent-chat` for the chat stream) attach the existing session's access token and target `RAGENT_URL`. Refresh still goes to apps/api (it owns refresh) regardless of which service the data call hit. The chat proxy is a separate route from the generic SSE proxy because chat is a **POST with a body** streaming back SSE — `EventSource` is out, and the generic GET stream proxy doesn't carry a request body; the client uses `fetch` + parser (`streamChat`). ragent's small surface (chat + corpora) is hand-typed on the web rather than wiring a second `openapi-typescript` source — worth it only if that surface grows.
...
# Phase 13 (ragent) — deploy + demo cost control

## Why a global daily cap instead of detecting/blocking VPNs?

Per-IP limits are the obvious first move, but they're trivially defeated by rotating IPs — and that's exactly what a VPN gives an abuser. The instinct is then to *detect* VPNs and block them, but reliable proxy/VPN detection needs a paid IP-reputation service (IPQualityScore, MaxMind Anonymous-IP) or fronting everything with Cloudflare's WAF, it's an ongoing arms race (residential proxies evade it), and it can false-positive on legitimate privacy-conscious users. For a portfolio demo that's the wrong amount of money and complexity. The cost we actually care about is the hosted-key bill, so we cap *that* directly: a **global** daily free-turn ceiling (one sentinel counter row) on top of the per-IP one. An attacker can rotate through a thousand IPs and still only consume the global pool, after which everyone falls through to BYOK — rotation buys them nothing. Per-IP keeps one casual visitor from draining the global pool for everyone; the global cap is the hard backstop. `free_turns_remaining` returns the *min* of the two.

## Why is the 402 raised before the SSE stream opens, and the turn recorded only on an answer?

Once `EventSourceResponse` starts streaming, the HTTP status is already 200 — you can't go back and say "payment required." So the quota is checked in the endpoint function, before the response object is returned, where a clean 402 (with a "add X-Provider-Key" message the UI can act on) is still possible. The *recording* is the opposite: it happens inside the generator, after the agent actually produces an answer, so a turn that errors out or streams nothing doesn't burn the visitor's free allotment. A demo turn only counts when it delivered value, and only the hosted-key path counts — BYOK turns spend the visitor's own key and skip the quota entirely.

## Why no `release_command` for ragent, and a manual corpora seed?

apps/api's seed is pure DB writes, so running it in the Fly release command (before traffic shifts) is safe. ragent's corpora seed is different: it runs real ingestion, which calls the embedding API — an *external* dependency. Putting that in a release command would let a transient OpenAI outage abort an otherwise-fine deploy. So ragent has no release command (it also never migrates — apps/api owns that), and the seed is a documented one-time `python -m promptforge_ragent.seed` step. It's idempotent and discovers Demo Corp from the shared DB, so it no-ops cleanly if apps/api hasn't been seeded yet — which also encodes the deploy order (api first).

## Why 1 GB Fly machines, not 512 MB?

litellm — the LLM/embedding client both processes use — is roughly 200 MB resident just imported. The web/chat process loads it (the agent makes several calls per turn) and so does the ingest worker (it embeds on ingest), and a one-off `python -m promptforge_ragent.seed` loads it again. On a 512 MB machine, the seed running alongside the worker tipped it over and the kernel OOM-killed the process (then rebooted the machine). The honest fix is to size the box for the dependency: 1 GB per process. Both still auto-stop to zero when idle, so the larger size only costs while something is actually running. This wasn't seed-specific — a real document upload would have OOM'd the worker the same way; the seed just surfaced it first.

## Why declare apps/api's `orgs`/`users` tables inside ragent (and why did tests miss it)?

ragent's ORM models have foreign keys to apps/api's `orgs` and `users`. To flush an insert, SQLAlchemy sorts tables by their FK dependencies and must *resolve* each FK's target table within the mapper's `MetaData` — and if the target isn't there it raises `NoReferencedTableError`, before any SQL runs. ragent doesn't own or migrate those tables, so they weren't in its metadata, so every insert of a Corpus/Conversation/etc. (and the seed) failed in production. It passed in tests only because the test conftest had registered stub `orgs`/`users` on the metadata to make `create_all` work — which silently doubled as the missing production declaration and hid the bug. The fix moves that declaration into production code (`models/_external.py`): partial tables with just the FK-referenced columns, registered on `Base.metadata` but never `create_all`'d in production (apps/api's migrations create the real ones). The broader lesson is the reason it escaped CI: **a test fixture should never provide schema that production also depends on** — doing so turns a prod-only failure green. With the declaration in production, the tests now exercise the same thing the deployed service does.

# Phase 11 (web) — Settings + API keys + BYOK

## Why hold the BYOK provider key in a sealed httpOnly cookie instead of localStorage?

The provider key is a real, billable secret — a user's OpenAI/Anthropic key. localStorage is readable by any script that runs on the page, so an XSS or a compromised dependency would exfiltrate it; it's the wrong place for a credential. The same BFF reasoning that keeps the API tokens out of the browser applies here: the key lives in an httpOnly cookie the browser JS can't read, sealed as a JWE so even a non-JS leak yields opaque ciphertext rather than a working key, and the server attaches it as `X-Provider-Key` when proxying chat. The browser only ever learns *whether* a key is configured and a short masked preview (first three + last four) so the user can confirm which key is stored — the key itself never travels back to the page after it's set.

## Why a separate `pf_byok` cookie rather than folding the key into the session cookie?

The session cookie is rewritten on every token refresh (rotated refresh token → re-seal), so anything stored in it rides the auth lifecycle. The provider key has nothing to do with auth: it should persist across refreshes untouched, be settable/removable on its own, and not be re-serialized on every 401-retry. A separate sealed cookie gives it an independent lifetime and a single clear responsibility, and means the BYOK set/clear path can't accidentally perturb the session. They share the JWE seal/unseal primitive (extracted to `lib/seal.ts`) but are otherwise unrelated — one secret to rotate (`WEB_SESSION_SECRET`), two cookies with different jobs.
...
# Phase 13 (web) — Playwright E2E

## Why mock the backend in-process with MSW instead of stubbing in the browser or running the real API?

The session cookie is sealed (JWE) server-side, so a test can't forge a logged-in state by setting a cookie from the browser — the only way to get a valid session is to let the real BFF auth route run. That rules out the usual "stub the network in the browser with `page.route`" approach: stubbing the browser→BFF calls would mean the BFF never runs, so login never seals a cookie and every authenticated page is unreachable. Running against the *live* Fly backends, on the other hand, makes CI slow (cold starts), flaky, and costly (LLM calls, demo quota), and couples a frontend test to backend uptime. The middle path is to run the **real** built web app — real BFF route handlers, real session sealing, real pages — and intercept the *server-side* `fetch` it makes to the upstream API with MSW running inside the Next server process. That exercises everything we actually wrote on the web side (proxying, auth/refresh, cookie sealing, SSE piping, rendering) against a deterministic fake backend, with no live dependency. The API and ragent each have their own server-side e2e suites; the web↔API contract is held by `openapi-typescript` generation.

## Why start MSW from `instrumentation.ts` behind an env gate + dynamic import?

MSW's Node interceptor has to be installed in the same process that issues the `fetch` calls — i.e. the Next server — and `instrumentation.ts`'s `register()` is Next's supported once-at-startup hook for exactly that kind of process-wide setup. It's gated on a test-only flag (`NEXT_PUBLIC_API_MOCKING==='enabled'`) and the node runtime (`NEXT_RUNTIME==='nodejs'`, since `register()` also runs on edge), and the mock is pulled in with a **dynamic `import()`** so that `msw` and the handlers are never evaluated — and never bundled into the running path — in a normal build or in production. Flip the flag on only from Playwright's `webServer.env`; a plain `next build && next start` (or the Vercel deploy) never touches it. The upstream fetches already use `cache: 'no-store'` (the BFF sets it; the share page sets it), which also means `next build` can't bake a response before MSW starts — the interception is live on every request.
...
## Why Vercel's GitHub integration instead of a GitHub Actions deploy job (like api/ragent)?

apps/api and apps/ragent deploy from GitHub Actions because Fly deploys are a CLI/Docker step we drive ourselves (build the image, run the release command, scale processes) — Actions is where that belongs. Vercel is the opposite: it's the native host for Next.js and already watches the repo, so its own integration does the build, the preview-per-PR, the atomic promote-to-production, and the rollback — re-implementing that in Actions would be strictly worse and would duplicate the build. So web's `web.yml` runs only the quality gates (lint, typecheck, build, the Playwright suite) and Vercel owns the deploy. The one non-obvious setting that makes it work in this monorepo is **Root Directory = `apps/web`**: without it Vercel builds from the repo root and finds no Next app. There's deliberately no `vercel.json` — the project settings (root directory + the three env vars) are all it needs, and config-in-the-dashboard is the documented Vercel default for a single app.
