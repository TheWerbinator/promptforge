...
**Current position:** apps/api Phase 14 DONE — public share tokens landed. Generic `ShareToken` (migration 0008) is polymorphic over `resource_type` ∈ {`prompt`, `eval_batch`} via (resource_type, resource_id); token stored hashed (HMAC, like RefreshToken), plaintext returned once at create. Owner-facing CRUD (`POST/GET /api/v1/shares`, `DELETE /shares/{id}`, all writer-gated + tenant-scoped) + the API's only unauthenticated read: `GET /api/v1/public/share/{token}`, which hashes the incoming token, rejects revoked/expired, resolves the resource, and returns a **minimal public projection** (no org_id/created_by/provider_response). Prompt share → name + latest version body/vars; eval-batch share → suite name, status, pass_rate, per-case results (the demo "here's my eval report" flex). **215 tests pass** end-to-end (full suite, Docker up). Next: Phase 15 (seed demo data).

**Fresh-chat bootstrap:** if you're picking this up cold, read this file's Bootstrap section at the bottom first. Short version: 14 of 17 apps/api phases done; api + Postgres live on Fly+Neon; ~3 more apps/api phases (15-17) then apps/ragent + apps/web. INTERVIEW-NOTES.md has 30+ defendable "Why" entries — read before any architecture conversation.

**Last verified locally:** 2026-06-02 — full suite **215 passed** (205 prior + 10 new: 7 share-flow e2e + 3 share tenancy), no flake; ruff + ruff format + mypy --strict clean across 49 source files. Full suite needs Docker for testcontainers Postgres (was up).

---

## apps/api — 14 / 17 phases done (~82%)
...
- **49 source files.**
- **215 tests pass** (full suite, Docker up — 2026-06-02). Unit + integration (testcontainers models/queue/reaper) + tenancy (api-keys/prompts/shares) + e2e (auth/prompts/runs/evals incl. SSE + demo mode + share links).
- ruff + ruff format + mypy --strict clean across all 49 source files.
