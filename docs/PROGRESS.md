...
**Current position:** apps/api Phase 13 DONE — demo mode landed. `POST /demo/login` mints a read-only session on the seeded Demo Corp workspace (no signup); `GET /demo/quota` reports remaining free runs. **Free-taste model:** demo visitors browse all seeded content for free (no quota), and get `demo_free_runs` (default 5) real hosted-key single runs **per client IP per day** before a 402 asks them to BYOK (`X-Provider-Key` header); BYOK bypasses the quota. Per-IP counter stored hashed (HMAC) in new `demo_usage` table (migration 0007). Read-only enforced via a `require_writer` gate (owner/member only) on every mutating route — demo blocked from prompt/version/suite/case writes, eval batch runs, and api-keys. slowapi wired for the first time: `/demo/login` rate-limited (default 5/min, forwarded-IP-aware key func). Lean refresh-token reaper (`services/maintenance.reap_expired_refresh_tokens`) runs periodically in the worker. **205 tests pass** end-to-end (full suite, Docker up). Next: Phase 14 (public share tokens).

**Fresh-chat bootstrap:** if you're picking this up cold, read this file's Bootstrap section at the bottom first. Short version: 13 of 17 apps/api phases done; api + Postgres live on Fly+Neon; ~4 more apps/api phases (14-17) then apps/ragent + apps/web. INTERVIEW-NOTES.md has 30+ defendable "Why" entries — read before any architecture conversation.

**Last verified locally:** 2026-06-02 — full suite **205 passed** (198 prior + 7 new: 6 demo-flow e2e + 1 reaper integration), no flake (demo-flow file run 3× consecutive); ruff + ruff format + mypy --strict clean across 46 source files. Full suite needs Docker for testcontainers Postgres (was up).

---

## apps/api — 13 / 17 phases done (~76%)
...
- **46 source files.**
- **205 tests pass** (full suite, Docker up — 2026-06-02). Unit + integration (testcontainers models/queue/reaper) + tenancy + e2e (auth/prompts/runs/evals incl. SSE + demo mode).
- ruff + ruff format + mypy --strict clean across all 46 source files.
