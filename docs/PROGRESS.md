...
**Current position:** apps/api Phase 15 DONE — idempotent demo seed landed (`promptforge_api/seed.py`, runnable via `python -m promptforge_api.seed`). Makes `/demo/login` work end-to-end: seeds the demo user (unusable password — reachable only via `/demo/login`) + Demo Corp org + DEMO membership, 3 realistic prompts (Support Reply Drafter w/ 2 versions, SQL Explainer, Release Notes Writer), 4 canned runs (incl. one failed run to show error handling), an eval suite + 3 cases + a finished batch with a 2/3 pass-rate result set, and 2 stable public share links (`demo-prompt-support-reply`, `demo-eval-support-quality`) so the README can point at a live prompt + a live eval report. Logic is in-package (testable/typed/covered); every section is get-or-create so it's safe on every deploy. **218 tests pass** end-to-end. Next: Phase 16 (deploy + smoke — wire seed into release, structlog + OTel).

**Fresh-chat bootstrap:** if you're picking this up cold, read this file's Bootstrap section at the bottom first. Short version: 15 of 17 apps/api phases done; api + Postgres live on Fly+Neon; ~2 more apps/api phases (16-17) then apps/ragent + apps/web. INTERVIEW-NOTES.md has 30+ defendable "Why" entries — read before any architecture conversation.

**Last verified locally:** 2026-06-02 — full suite **218 passed** (215 prior + 3 new: 2 seed e2e + 1 seed integration), no flake (auth+seed files run 3× consecutive). One transient teardown-race failure observed once on a full run, not reproducible. ruff + ruff format + mypy --strict clean across 50 source files. Full suite needs Docker for testcontainers Postgres (was up).

---

## apps/api — 15 / 17 phases done (~88%)
...
- **50 source files.**
- **218 tests pass** (full suite, Docker up — 2026-06-02). Unit + integration (testcontainers models/queue/reaper/seed) + tenancy (api-keys/prompts/shares) + e2e (auth/prompts/runs/evals incl. SSE + demo mode + share links + seed).
- ruff + ruff format + mypy --strict clean across all 50 source files.
